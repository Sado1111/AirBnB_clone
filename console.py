#!/usr/bin/python3
"""A commandline interpreter for commands specific to the AirBnB project"""
...
count_user, count_base_model, count_state, count_city,
count_amenity, count_place, count_review
...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        ...
        and save it to JSON file and Print its id
        """
        if not line:
    ...
    def do_all(self, line):
        """
        Print string representation of all instances based/not on class name
        """
        tokens = line.split()
        if not tokens:
            ...
            print(
                [str(obj) for obj in FileStorage.objects_copy.values()
                    if type(obj) == globals()[tokens[0]]])
    ...
    def do_update(self, line):
        ...
        if instance_attribute not in FileStorage.objects_copy:
            ...
            return
        setattr(
            FileStorage.objects_copy[instance_attribute], tokens[2], tokens[3])
        FileStorage.save(FileStorage)
